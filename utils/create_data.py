...
def create_dummy_study_data(name="dummy_data.csv", seed=42, num_samples=100, min_value=[0.5, 1], max_value=[12, 100]):
    """
    ...
    field1 = np.round(np.random.uniform(min_value[0], max_value[0], num_samples), 2)

    # Generate percentiles based on a loose correlation to study time
    # Add some noise to make it more realistic
    noise = np.random.normal(0, 10, num_samples)
    field2 = np.clip((field1 * 7) + noise, min_value[1], max_value[1]).astype(int)
